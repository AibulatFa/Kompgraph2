                ...
                override fun onResourceReady(resource: Bitmap, transition: Transition<in Bitmap>?) {
                    seekbar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                        override fun onProgressChanged(seekBar: SeekBar, progress: Int, b: Boolean) {
                            imageView.setImageBitmap(adjustedContrast(resource, progress.toDouble()/1000))
                        }
    ...
    private fun adjustedContrast(src: Bitmap, value: Double): Bitmap? {
        ...
        for (x in 0 until width) {
            for (y in 0 until height) {
                ...
                A = Color.alpha(pixel)
                R = Color.red(pixel)
                R = (((R / 255.0 - 0.5) * contrast) * 255.0).toInt()
                if (R < 0) {
                    R = 0
                } else if (R > 255) {
                    R = 255
                ...
                G = Color.green(pixel)
                G = (((G / 255.0 - 0.5) * contrast) * 255.0).toInt()
                if (G < 0) {
                    G = 0
                } else if (G > 255) {
                    G = 255
                ...
                B = Color.blue(pixel)
                B = (((B / 255.0 - 0.5) * contrast) * 255.0).toInt()
                if (B < 0) {
                    B = 0
                } else if (B > 255) {
                    B = 255
